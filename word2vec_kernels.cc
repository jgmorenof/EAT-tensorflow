 ...
 private:
  ...
  Status Init(Env* env, const string& filename) {
    ...
    while (ScanWord(&input, &w)) {
      ...
      if (w.find("wikipage_") != std::string::npos and w.find("[[") != std::string::npos and w.find("]]") != std::string::npos and w.find("|") != std::string::npos) {
        int pos = w.find("|");
        string w2 = w.substr(2,pos-2);
        ++(word_freq[w2]);
        ++corpus_size_;        
        w = w.substr(pos+1,w.length()-w2.length()-5);
      }
      ++(word_freq[w]);
      ++corpus_size_;
    }
